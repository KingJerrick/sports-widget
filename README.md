...
```
┌──────────────────────────────────────────────────────┐
│ 刷新 ▍今 六 日 一 二 三 四       9/11 09:38   抓取  │
├──────────────────────────────────────────────────────┤
│ ╭────╮ F1 · 西班牙站                今天 19:30      │
...
卡片底色表示状态，越暗越接近结束。**已结束的置顶**，右侧显示赛果，只保留 24 小时内的：
...
**比分一律是「我们-对手」**（`6-2` 就是道奇得 6 分），不跟主客变 —— 标题的
`@ / vs` 写法本来就在变，比分再跟着调顺序就读不明白了。

**MotoGP 的完赛卡片没有冠军**，和 F1 不一样。它的成绩接口是非公开路径
（试了 9 条全 400/404），这是数据源的限制，不是 bug。

**三个底色的色值是按 Lab 色差定的，别凭感觉改** —— 第一版只做到 ΔE 3.0
（人眼阈值约 2.3），看着「差不多」实际分不出来；深色那版更糟，`#0A1024`
比底板还暗，卡片像挖了个洞。详见 `drawable/bg_card.xml` 的说明。
...
**问题**：后端每 6 小时才抓一次。所以「刷新」拉到的往往还是同一份数据 ——
连点十次也还是那几个小时前生成的那份，看着像按钮坏了。

**做法**：App 里的「立即触发 GitHub 抓取」会调 GitHub 的 `workflow_dispatch`
接口**让后端立刻开跑**。跑完（一次 Actions 实测一两分钟）回来点「刷新」就拿到新的了。

**这是一个两步操作，不是一步。** 小组件上也是两个按键，一左一右：

```
[刷新] ▍今 六 日 一 二 三 四   9/15 14:30   [抓取]
  │                                          │
  │ 手机 GET 一份现成的 calendar.json         │ 让 GitHub Actions 现在开跑
  │ 不碰 Actions                             │ 不拉数据
  ▼                                          ▼
拿到后端**上一次**抓的那份              状态行变成「✅ 已触发 15:32」
```

想拿到更新的数据，就是：**点「抓取」→ 等一两分钟 → 点「刷新」**。

> ⚠️ **别指望「抓取」之后会自动取数。** 早先的版本是这么做的（触发后排一个
> WorkManager 延时任务，2 分钟后自己回来取），但那个延时任务在 Doze 和各家的
> 省电策略下想推迟就推迟，实测经常根本不执行 —— 表现是「点了触发、GitHub 也真跑了，
> 但手机上永远没自动拿到新数据」，不报错、不崩，最难查。
> 与其留一个「有时候灵」的自动行为，不如把两件事拆成两个按键，行为完全可预期。

要启用「抓取」，得建一个 GitHub token 填进 App：

1. GitHub → **Settings → Developer settings → Personal access tokens →
   Fine-grained tokens → Generate new token**
2. **Repository access** 只选 `KingJerrick/sports-widget` 这一个仓库
3. **Permissions → Repository permissions → Actions** 选 **Read and write**
   （只给这一个权限，不用给 Contents）
4. 生成的 `github_pat_…` 粘进 App 设置里那一栏，点「立即触发 GitHub 抓取」验证

**不填 token 也完全能用** —— 只是「刷新」拉到的还是后端上一次抓的那份，
和以前一样。这个功能是纯增量。

**token 存哪、为什么**：只存在**这台手机**上（app 私有的 SharedPreferences，
`allowBackup=false`，不会被系统备份带走）。这是唯一可行的位置 ——
workflow dispatch 必须认证，而放进构建期配置会被打进 APK（谁装了谁就有），
放进仓库就直接公开了。存在设备上，泄漏面就只剩这一台手机。

> ⚠️ 想撤销就直接去 GitHub 删掉那个 token，App 这边留着的那个会立刻失效。

**仓库是从哪知道的**：不新增「仓库地址」输入框 —— 你填的数据地址里本来就带着
仓库信息（jsDelivr / gitmirror / raw.githubusercontent / github 网页地址四种写法都认）。
认不出来时按钮会明确告诉你，而不是猜一个错的仓库去请求。
...
**第一次加到桌面时可能是空的**，同时顶栏显示「尚未获取数据」—— 这是正常的，后台任务正在拉数据，等十几秒或点一下左边的「刷新」就会出来。

### 交互

| 点哪里 | 做什么 |
|---|---|
| 左端 **刷新** | 去 GitHub 拉一份现成的数据，状态行先变成「刷新中…」。**不碰 Actions** |
| 右端 **抓取** | 让 GitHub Actions 现在开跑，状态行先变成「触发中…」，跑完变成「✅ 已触发 15:32」或失败原因。**不拉数据** |
| **任意一张卡片** | 打开 App 的完整赛程页 |
| 七天条 / 四周空白 | 打开 App（七天条本身没有点击事件） |
| 卡片列表区**上下滑** | 看后面几天的比赛 |

两个按键的完整用法见 [「让后端现在就去抓」](#让后端现在就去抓可选) —— 简言之：
想拿更新的数据，点「抓取」→ 等一两分钟 → 点「刷新」。

> **两个按键为什么一左一右。** 触控区只有 32×22dp，远低于安卓建议的 48dp ——
> 小组件总共才 110dp 高，顶栏再加高就没地方放卡片了。所以防误触靠的是**距离**不是尺寸：
> 两个按键之间隔着整条七天条，而七天条没有任何点击事件，点歪了最多落到根布局上（打开 App），
> 不会误触发另一个按键。
>
> 也正因为有了第二个按键，点击**不再挂在整条顶栏上**。那是单按键时代的做法
> （那时顶栏只有「刷新」一个动作，整条可点等于把触控区从 18dp 放大到整条）；
> 有了两个动作之后再整条可点，就等于两个都不准。
...
| 高度 | 可见卡片 |
|---|---|
| 110dp（4×2） | 约 1.6 张，其余往下滑 |
| 150dp | 约 2.6 张 |
| 180dp（上限） | 约 3.2 张 |

（比上一版各少 0.1~0.2 张：顶栏为了放得下两个按键从 18dp 长到了 24dp。
这笔账是划算的 —— 18dp 是「将将能点到」，24dp 是「点得准」。）
...
- **④ 卡片图标** —— 给每个分组传一张图（见下）
- **⑤ 设置** —— 数据地址、刷新间隔、GitHub token，加三个按键：**保存**（只存设置）/
  **刷新**（只去拉数据）/ **测试连接**（只报告不落库），下面还有**立即触发 GitHub 抓取**（只触发）

> 每个按键只做一件事，边界是「联网做什么」：**保存不联网**、**刷新不触发 Actions**、
> **触发不拉数据**。以前是「保存并刷新」和「触发 + 顺手拉一次 + 排个延时任务」，
> 一个按键里塞了三件事，出问题时完全看不出是哪一步没成。
...
这个间隔只管**自动拉取**（`RefreshScheduler` 的周期任务）。两个按键都不受它约束：
「刷新」是随时可以点的一次性任务，「抓取」更是不经过它。

### 数据超过 24 小时没更新会怎样

顶栏那行会变成琥珀色的 `⚠ 9/08 09:38`。

（点过「抓取」之后，这一行会先被那条触发结果占着 —— 「✅ 已触发 15:32」或「❌ 没配 token」——
一直到你下次点「刷新」为止。它和上面那个琥珀色是两种不同的提醒，所以触发**失败**时
那行也是琥珀色的，成功则保持灰色。）
...
| 加到桌面后一直空白 | 点一下左端的「刷新」；还不行就打开 App 看 ③ 数据源状态 |
| 点了「刷新」数据还是旧的 | 正常。后端每 6 小时才抓一次，刷新只是去把现成的那份拿回来。要更新的就先点「抓取」、等一两分钟再点「刷新」 |
| 点了「抓取」状态行一直是时间戳 | 状态行只在跑完之后才写结果。没网时这个任务会被 WorkManager 压着等网络，恢复后才跑；也可能是没配 token，那样它会显示「❌ 没配 token」 |
...
| 数据不刷新 | ① 看顶栏的时间有没有往前走（那行如果还显示着「✅ 已触发…」，点一次「刷新」把它清掉）② 看 App 里 ③ 的状态 ③ 试着在设置里填个自定义地址 |
...
└── app/src/main/
    ...
    │   ├── RefreshWorker.kt     # Worker + Scheduler + 刷新编排（只拉数据）
    │   ├── TriggerWorker.kt     # Worker + Scheduler + 触发编排（只触发 Actions）
