...
**问题**：后端每 6 小时才抓一次。所以「刷新」按钮拉到的往往还是同一份数据 ——
连点十次也还是那几个小时前生成的那份，看着像按钮坏了。

**做法**：小组件顶栏的 ↻ 和 App 里的「立即触发 GitHub 抓取」会让 App 调
GitHub 的 `workflow_dispatch` 接口**让后端立刻开跑**，然后排一个 WorkManager
延时任务，**2 分钟后自动回来取新数据**（一次 Actions 实测要一两分钟）。

要启用它，得建一个 GitHub token 填进 App：
