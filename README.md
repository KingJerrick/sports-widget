...
左侧的标会去网上拉真实队标（皇马队徽、F1 标志……），拉到之前先显示上表里的字母块。
...
| 现象 | 原因 / 怎么办 |
|---|---|
| **卡片左侧是彩色字母块，不是队标** | 图标还没拉下来。首次安装、离线、或者原站拦了都会这样。**这是降级不是故障** —— 字母块用的是分类色，一眼还是能认出是哪一类。联网点一次 ↻ 就该变成真队标 |
...
**队标是运行时去原站拉的，不进仓库也不打进 APK。** 队标是别人的商标，扔进公开仓库算再分发；打进 APK 又意味着构建时要联网下载，本机用 Android Studio 构建就会缺资源。所以 App 拉一次、缓存到本地，之后完全离线可用；拉不到就退回分类色字母块，卡片不会开天窗。`setImageViewBitmap` 会把整张位图塞进 Binder 事务（全进程共享约 1MB），所以拉下来的图先缩到 96px 再缓存 —— IG 那张原图解码后是 360KB，五张一起就顶到上限了。
